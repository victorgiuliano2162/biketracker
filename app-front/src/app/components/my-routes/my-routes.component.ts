...
export class MyRoutesComponent {
  ...
  hoverIndexMap = new Map<string, number | null>();
  ...
  onChartHover(routeId: string, index: number | null): void {
    this.hoverIndexMap.set(routeId, index ?? null);
  }
  ...
  deleteRoute(route: RouteResponse, event: MouseEvent): void {
    ...
    this.routeService.deleteRoute(route.id).subscribe({
      next: () => {
        // Remove localmente sem recarregar a página
        this.routes = this.routes.filter((r) => r.id !== route.id);
        this.totalElements--;

        // Limpa caches associados
        this.replayCache.delete(route.id);
        this.chartDataCache.delete(route.id);
        this.avgElevCache.delete(route.id);
        this.chartInstances.get(route.id)?.destroy();
        this.chartInstances.delete(route.id);
...
toggleRoute(routeId: string): void {
  ...
  this.routeService.getReplay(routeId).subscribe({
    next: (replay) => {
      ...
      this.chartDataCache.set(routeId, this.buildChartData(replay.points));
  ...
  private renderChart(routeId: string, canvas: HTMLCanvasElement): void {
    ...
    const chart = new Chart(canvas, {
      ...
      options: {
        ...
        onHover: (_, elements) => {
          if (!elements.length) {
            this.hoverIndexMap.set(routeId, null);
            return;
          }
          this.hoverIndexMap.set(routeId, elements[0].index);
        },
      },
    });
...
${trkpts}
  ...
  togglePrivacy(route: RouteResponse, event: MouseEvent): void {
    ...
    this.routeService.toggleVisibility(route.id).subscribe({
      next: (updated) => {
        const index = this.routes.findIndex((r) => r.id === updated.id);
        if (index !== -1) this.routes[index] = updated;
      },
